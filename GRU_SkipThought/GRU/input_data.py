    ...
    def __init__(self,
        ...
        for item in data:
            if self.sub_category in item['label'][self.super_category]:
                self.dataset['positive'].append((item['sentence_embeddings'], item['id']))
            else:
                self.dataset['negative'].append((item['sentence_embeddings'], item['id']))
        ...
        self.testset_ids = [item[1] for item in self.testset_input]
